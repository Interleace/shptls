...
<b>Shopowner:</b> kabbalabuch.info</pre>
    ...
    def update_order(self, order_id, data):
        """
        Aktualisiert eine Bestellung mit den gegebenen Daten.
        :param order_id: ID der zu aktualisierenden Bestellung
        :param data: Dictionary mit den zu aktualisierenden Feldern
        """
        try:
            # Holen der aktuellen Bestelldaten
            current_order = self._make_request('GET', f'orders/{order_id}')

            # Zusammenführen der Änderungen
            updated_order = {
                'billing': {**current_order.get('billing', {}), **data.get('billing', {})},
                'shipping': {**current_order.get('shipping', {}), **data.get('shipping', {})},
                'customer_note': data.get('customer_note', current_order.get('customer_note', ''))
            }
